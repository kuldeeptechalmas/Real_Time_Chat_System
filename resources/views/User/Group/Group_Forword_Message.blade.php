...
<div class="d-flex text-white" onclick="SelectedForwordUserGroup(this)" data-id="{{ $item->sendersData->id }}" style="border-radius: 20px;background-color:#212529;position: relative;padding: 16px;margin: 4px;">
    <div class="d-flex justify-content-center" style="height: 37px;width: 37px;">
        ...
        @else
        <div style="height: 37px;width: 37px;"><img style="height: 100%;width: 100%;border-radius: 114px;object-fit: cover;" data-bs-toggle="modal" data-bs-target="#imageshowmodel" onclick="imagesetshow('{{ $item->sendersData->name }}','female.png','{{ $item->sendersData->phone }}','{{ $item->sendersData->email }}')" src="{{ asset('img/female.png') }}" alt=""></div>
        @endif
        @endif
    </div>
    <div style="width: 100%;">
